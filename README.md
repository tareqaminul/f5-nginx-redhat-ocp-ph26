...
> **OpenShift-adapted fork of [`F5EMEA/oltra`](https://github.com/F5EMEA/oltra).**
> All `kubectl` commands have been converted to `oc`, and OpenShift-specific
> requirements (SecurityContextConstraints, the missing `setup/` directory,
> image and service caveats) are documented in **[`ocp-changes.md`](ocp-changes.md)**.
> OpenShift helper resources live in **[`openshift/`](openshift/)**.
>
> ⚠️ Read `ocp-changes.md` before running any lab — the command swap alone is
> not sufficient to run on OpenShift.
