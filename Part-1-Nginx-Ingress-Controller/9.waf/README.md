...
oc apply -f syslog2.yaml
```

Create the DoS protected resource configuration:
```
oc apply -f apdos-protected.yaml
```

Create the App Protect DoS policy and log configuration:
```
oc apply -f apdos-policy.yaml
oc apply -f apdos-logconf.yaml
```
...
```
oc exec -it <SYSLOG_POD> -- cat /var/log/messages
```

To check the access logs in the syslog pod:
```
oc exec -it <SYSLOG_POD_2> -- cat /var/log/messages
```
